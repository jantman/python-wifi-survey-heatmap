...
# For getting wireless interfaces
from libnl.attr import nla_data, nla_len, nla_get_string, nla_get_u8, nla_get_u16, nla_get_u32, nla_parse, nla_put_u32
from libnl.error import errmsg
...
class Scanner(object):
    ...
    def _callback_dump(self, msg, results):
        ...
        if nla_parse_nested(bss, nl80211.NL80211_BSS_MAX, tb[nl80211.NL80211_ATTR_BSS], bss_policy):
            logger.warning('Failed to parse nested attributes for an access point!')
            return libnl.handlers.NL_SKIP
        ...
        if not bss[nl80211.NL80211_BSS_INFORMATION_ELEMENTS]:
            logger.warning('No additional information available for an access point!')
            return libnl.handlers.NL_SKIP
    ...
    def _do_scan_trigger(self, if_index, driver_id, mcid):
        ...
        ret = nl_socket_add_membership(self._nl_sock, mcid)  # Listen for results of scan requests (aborted or new results).
        if ret < 0:
            return ret
        ...
        libnl.handlers.nl_cb_set(cb, libnl.handlers.NL_CB_VALID, libnl.handlers.NL_CB_CUSTOM, self._callback_trigger, results)
        libnl.handlers.nl_cb_err(cb, libnl.handlers.NL_CB_CUSTOM, self._error_handler, err)
        libnl.handlers.nl_cb_set(cb, libnl.handlers.NL_CB_ACK, libnl.handlers.NL_CB_CUSTOM, self._ack_handler, err)
        ...
        if ret < 0:
            return ret
        while err.value > 0:
            logger.debug('Retrieving NL80211_CMD_TRIGGER_SCAN acknowledgement...')
            ret = nl_recvmsgs(self._nl_sock, cb)
            if ret < 0:
                return ret
        if err.value < 0:
            logger.warning('Unknown error {0} ({1})'.format(err.value, errmsg[abs(err.value)]))

        # Block until the kernel is done scanning or aborted the scan.
        while results.value < 0:
            logger.debug('Retrieving NL80211_CMD_TRIGGER_SCAN final response...')
            ret = nl_recvmsgs(self._nl_sock, cb)
            if ret < 0:
                return ret
        if results.value > 0:
        ...
        return nl_socket_drop_membership(self._nl_sock, mcid)  # No longer need to receive multicast messages.
    ...
    def _do_scan_results(self, if_index, driver_id, results):
        ...
        cb = libnl.handlers.nl_cb_alloc(libnl.handlers.NL_CB_DEFAULT)
        libnl.handlers.nl_cb_set(cb, libnl.handlers.NL_CB_VALID, libnl.handlers.NL_CB_CUSTOM, self._callback_dump, results)
    ...
    def scan_all_access_points(self):
        ...
        try:
            info = struct.unpack('16sI', fcntl.ioctl(sock.fileno(), 0x8933, pack))
        except OSError:
            return logger.warning('Wireless interface {0} does not exist.'.format(self._interface_name))
        finally:
            sock.close()
        if_index = int(info[1])
        ...
        for i in range(2, -1, -1):  # Three tries on errors.
            ...
            if ret < 0:
                logger.debug('do_scan_trigger() returned {0}, retrying in 5 seconds ({1}).'.format(ret, i))
                time.sleep(5)
            ret = self._do_scan_results(if_index, driver_id, results)
            if ret < 0:
                logger.debug('do_scan_results() returned {0}, retrying in 5 seconds ({1}).'.format(ret, i))
                time.sleep(5)
                continue
            break
        if not results:
        ...
        for ap in results:
            for prop in results[ap]:
                if isinstance(results[ap][prop], datetime.timedelta):
                    results[ap][prop] = int(results[ap][prop].microseconds)/1000
    ...
    def _iface_callback(self, msg, _):
        ...
        # Now it's time to grab the data, we start with the interface index as universal identifier
        ...
        if tb[nl80211.NL80211_ATTR_IFNAME]:
            iface_data['name'] = nla_get_string(tb[nl80211.NL80211_ATTR_IFNAME]).decode('ascii')
        ...
        if tb[nl80211.NL80211_ATTR_WIPHY_TX_POWER_LEVEL]:
            iface_data['tx_power'] = nla_get_u32(tb[nl80211.NL80211_ATTR_WIPHY_TX_POWER_LEVEL])/100 # mW
        ...
        if tb[nl80211.NL80211_ATTR_STA_INFO]:
            ...
            sinfo = dict((i, None) for i in range(nl80211.NL80211_STA_INFO_MAX))
            nla_parse_nested(sinfo, nl80211.NL80211_STA_INFO_MAX, tb[nl80211.NL80211_ATTR_STA_INFO], None)
            ...
            if sinfo[nl80211.NL80211_STA_INFO_TX_BITRATE]:
                ...
                nla_parse_nested(rinfo, nl80211.NL80211_RATE_INFO_MAX, sinfo[nl80211.NL80211_STA_INFO_TX_BITRATE], None)
                if rinfo[nl80211.NL80211_RATE_INFO_BITRATE]:
                    iface_data['bitrate'] = nla_get_u16(rinfo[nl80211.NL80211_RATE_INFO_BITRATE])/10

        # BSS info
        if tb[nl80211.NL80211_ATTR_BSS]:
            # Need to unpack the data
            binfo = dict((i, None) for i in range(nl80211.NL80211_BSS_MAX))
            nla_parse_nested(binfo, nl80211.NL80211_BSS_MAX, tb[nl80211.NL80211_ATTR_BSS], None)
    ...
    def update_iface_details(self, cmd):
        ...
        self._nl_sock = nl_socket_alloc()  # Creates an `nl_sock` instance.
        ret = genl_connect(self._nl_sock)  # Create file descriptor and bind socket.
        if ret < 0:
            reason = errmsg[abs(ret)]
            logger.error('genl_connect() returned {0} ({1})'.format(ret, reason))
            return {}

        # Now get the nl80211 driver ID. Handle errors here.
        driver_id = genl_ctrl_resolve(self._nl_sock, b'nl80211')  # Find the nl80211 driver ID.
        if driver_id < 0:
            reason = errmsg[abs(driver_id)]
            logger.error('genl_ctrl_resolve() returned {0} ({1})'.format(driver_id, reason))
            return {}
        ...
        if self.if_idx == None:
            ...
            nla_put_u32(msg, nl80211.NL80211_ATTR_IFINDEX, self.if_idx)  # This is the interface we care about.
            #nla_put_u32(msg, nl80211.NL80211_ATTR_MAC, 2199023255552)

        # Add the callback function to the self._nl_sock.
        nl_socket_modify_cb(self._nl_sock, NL_CB_VALID, NL_CB_CUSTOM, self._iface_callback, False)

        # Now send the message to the kernel, and get its response, automatically calling the callback.
        ret = nl_send_auto(self._nl_sock, msg)
        if ret < 0:
            reason = errmsg[abs(ret)]
            logger.error('nl_send_auto() returned {0} ({1})'.format(ret, reason))
            return {}
        logger.debug('Sent {0} bytes to the kernel.'.format(ret))
        ret = nl_recvmsgs_default(self._nl_sock)  # Blocks until the kernel replies. Usually it's instant.
        if ret < 0:
            reason = errmsg[abs(ret)]
            logger.error('nl_recvmsgs_default() returned {0} ({1})'.format(ret, reason))
            return {}
